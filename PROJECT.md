...
- **20:46** 🗑️ [deprecate] 배포일지 워크플로우 폐기
- **20:44** 🐛 [fix] 배포일지 업로드 워크플로우 권한 추가
- **20:10** ✨ [feat] 배포일지 일괄 업로드 워크플로우
- **19:33** ✨ [feat] 상세 타임라인 일별 헤더 자동 생성
...
- **16:43** 🐛 [fix] PROJECT.md 자동 업데이트 워크플로우 섹션명 수정
...
- **13:25** 🐛 [bugfix] Cron 중복 실행 방지 (날짜별 락 키)
- **13:25** 🐛 [bugfix] Cron 중복 실행 방지 (날짜별 락 키)
- **13:24** 🐛 [bugfix] Cron 중복 실행 방지 (날짜별 락 키)
- **07:15** ⚡ [improvement] Queue 병렬 처리 최적화 (Promise.all)
- **07:15** ⚡ [improvement] Queue 병렬 처리 최적화 (Promise.all)
- **07:15** ⚡ [improvement] Queue 병렬 처리 최적화 (Promise.all)
- **07:02** 📝 [docs] 포스팅 생성 및 구글 드라이브 규칙 추가
- **04:44** 🔹 [other] 클라우드플레어 워커 자동 배포
- **04:44** 🐛 [fix] Queue consumer 설정 추가
- **04:44** 🐛 [fix] Queue consumer 설정 추가
- **04:36** 🔹 [other] 클라우드플레어 워커 자동 배포
- **04:36** 🔹 [debug] 저장소 저장 디버그 로그 추가
- **04:36** 🔹 [debug] 저장소 저장 디버그 로그 추가
- **04:36** 🔹 [debug] 저장소 저장 디버그 로그 추가
- **04:23** 🔹 [other] 클라우드플레어 워커 자동 배포
- **04:23** 🐛 [fix] ARCHIVE_SHEET_NAME 저장소로 수정
- **04:23** 🐛 [fix] ARCHIVE_SHEET_NAME 저장소로 수정
- **04:19** 🔹 [other] 클라우드플레어 워커 자동 배포
- **04:19** ✨ [feat] 포스팅 URL 자동 생성 및 저장 기능 추가
- **04:19** ✨ [feat] 포스팅 URL 자동 생성 및 저장 기능 추가
- **04:19** ✨ [feat] 포스팅 URL 자동 생성 및 저장 기능 추가
- **04:10** 🔹 [other] 클라우드플레어 워커 자동 배포
...
- **03:53** 🔹 [chore] Worker 재배포 트리거
- **03:53** 🔹 [chore] Worker 재배포 트리거
- **03:53** 🐛 [fix] Secret 설정 단계 주석 처리
- **03:51** 🔹 [revert] 정상 작동 버전으로 복원 (e2606cb)
- **03:51** 🔹 [revert] 정상 작동 버전으로 복원 (e2606cb)
- **03:50** 🔹 [revert] 이전 버전으로 복원 (빈 페이지 문제 해결)
...
- **03:25** 🐛 [fix] 빈 try 블록 제거 (문법 에러 수정)
- **03:25** 🐛 [fix] 빈 try 블록 제거 (문법 에러 수정)
- **03:24** 🔒 [security] 삭제 API 제거, KV 백업, CORS 추가
- **03:06** 📝 [docs] PROJECT.md 최신 상태 반영
...
- **23:36** ✨ [feature] 통계 첫 방문 시 자동 생성 (상호명)
- **23:36** ✨ [feature] 통계 첫 방문 시 자동 생성 (상호명)
- **23:21** ✨ [feature] 통계ID 자동 입력 기능 추가
- **23:21** ✨ [feature] 통계ID 자동 입력 기능 추가
...
- **23:05** ⚡ [improvement] Google Sheets '통계ID' 컬럼 연동
- **23:05** ⚡ [improvement] Google Sheets '통계ID' 컬럼 연동
- **23:01** ✨ [feature] Umami Cloud 전환 + D1/Plausible 제거
- **23:01** ✨ [feature] Umami Cloud 전환 + D1/Plausible 제거
...
- **20:42** 🐛 [bugfix] Worker 파일 손상 수정
- **20:42** 🐛 [bugfix] Worker 파일 손상 수정
- **20:38** ✨ [feature] SPA UI 파일 추가
- **20:36** ♻️ [refactor] SPA 방식 통계 구현 (파일 분할)
- **20:34** 🔹 [Revert "refactor] SPA 방식 통계 구현 (파일 분할)"
- **20:34** 🔹 [Revert "refactor] SPA 방식 통계 구현 (파일 분할)"
- **20:34** 🔹 [Revert "refactor] SPA 방식 통계 구현 (파일 분할)"
- **20:31** ♻️ [refactor] SPA 방식 통계 구현 (파일 분할)
- **19:48** 🐛 [bugfix] Worker 이름을 make-page-subdomain으로 수정
- **19:48** 🐛 [bugfix] Worker 이름을 make-page-subdomain으로 수정
- **19:15** 🔹 [other] 클라우드플레어 워커 자동 배포
- **19:14** 🐛 [bugfix] wrangler.toml 경로 수정
- **19:11** 🔹 [other] 클라우드플레어 워커 자동 배포
- **19:07** 🐛 [bugfix] queue consumer 중복 제거
- **19:07** 🐛 [bugfix] queue consumer 중복 제거
- **19:06** 🐛 [bugfix] KV namespace ID 수정
- **18:58** 📦 [deployment] 재배포 트리거
- **18:57** 🔹 [other] D1 데이터베이스 설정
- **18:57** 🏗️ [infra] D1 스키마 생성 추가
- **18:55** 🏗️ [infra] D1 analytics 데이터베이스 바인딩 추가
- **18:54** 🔹 [other] D1 데이터베이스 설정
- **18:50** 🏗️ [infra] D1 데이터베이스 설정 워크플로우 추가
- **18:45** ✨ [feature] D1 기반 통계 기능 추가
- **18:45** ✨ [feature] D1 기반 통계 기능 추가
- **18:44** ✨ [feature] D1 기반 통계 기능 추가
- **17:56** ✨ [feature] Umami 통계 자동 추가
- **17:56** ✨ [feature] Umami 통계 자동 추가
- **17:52** 🔹 [chore] umami credentials 파일 gitignore 추가
- **17:32** ⚡ [improvement] 포스트 표시 2개→1개로 변경
- **17:32** ⚡ [improvement] 포스트 표시 2개→1개로 변경
- **09:17** 🔹 [other] 클라우드플레어 워커 자동 배포
- **09:16** 🐛 [bugfix] 워크플로우 환경 지정 제거
- **09:14** 🐛 [bugfix] wrangler.toml 환경 구조 명시화 (크론 배포 수정)
- **09:13** 🐛 [bugfix] 크론 트리거 배포 환경 명시
...
- **21:28** 🚀 [performance] 저장소 시트 중복 읽기 제거
- **21:02** ✨ [feature] /test-cron 엔드포인트 추가
- **21:02** 🔹 [other] 클라우드플레어 워커 자동 배포
- **20:57** 🔹 [other] 클라우드플레어 워커 자동 배포
- **20:55** 🔹 [cleanup] 디버깅 로그 제거
...
- **20:51** 🔹 [debug] Gemini API 응답 로깅 추가
- **20:50** 🔹 [debug] Gemini API 응답 디버깅 추가
...
- **18:38** ⚡ [improvement] Gemini API timeout 120초로 증가
- **18:27** ⚡ [improvement] Git Hook + 간단 요약 자동화
- **16:12** 📝 [docs] 시스템 상태 자동 동기화 스크립트 추가
...
- **11:46** 🐛 [bugfix] Worker 빈 파일 복구
- **11:28** ♻️ [refactor] post1/post2 로직 완전 삭제
...
- **07:08** 🐛 [bugfix] test-sheet 엔드포인트 추가 (시트 데이터 디버깅용)
- **07:06** 🐛 [bugfix] test-sheet 엔드포인트 추가 (시트 데이터 디버깅용)
...
- **06:26** 🔹 [debug] 자세한 에러 로깅 추가
- **06:24** 📦 [deployment] Secret 재설정을 위한 재배포
...
- **23:56** 📦 [deployment] Worker 재배포 트리거
- **23:53** 🔹 [other] Merge branch 'main' of https://github.com/jeonwoohyun85/content-factory
- **23:53** 🔹 [other] Merge branch 'main' of https://github.com/jeonwoohyun85/content-factory
...
- **22:50** 🐛 [fix] 폴더 순환 로직 수정 (저장소 탭 기반)
- **22:38** 🐛 [fix] GitHub Actions에서 drive-to-sheets 배포 단계 제거
- **22:35** 🔹 [cleanup] drive-to-sheets 폐기 + wrangler.toml gid 수정
...
- **17:33** 🔹 [other] '바로가기' 컬럼 매핑 추가
- **17:27** 🔹 [other] Worker 설정 분리 및 package.json 추가
...
- **05:19** ⚡ [improvement] Posts 시트 기반 포스팅 저장으로 변경
- **05:18** ⚡ [improvement] Posts 시트 기반 포스팅 저장으로 변경
- **04:58** ⚡ [improvement] Posts 시트 기반 포스팅 저장으로 변경
...
- **02:07** ⚡ [improvement] Gemini 1.5 Pro → 3 Pro Preview 모델 변경
- **01:36** 📝 [docs] Gemini API 모델 정보 수정 (2.5 Flash, 3.0 Pro)
...
- **23:53** 🔹 [other] 📝 [문서] 깃허브 액션 목록 한글화 적용 확인을 위한 업데이트
- **23:49** 📝 [docs] rename all GitHub Actions workflows to Korean for better visibi…
- **23:47** 📝 [docs] localize workflow names to Korean completely
- **23:45** 🔹 [other] ✨ [feature] Complete automated posting system with Scheduled handler
- **23:45** 🔹 [other] ✨ [feature] Complete automated posting system with Scheduled handler
- **23:05** 📝 [docs] localize GitHub Actions workflow names to Korean
...
- **22:22** ⚡ [improvement] Retention Policy 제거 - 모든 포스트 보관
- **22:16** 🗑️ [deprecate] 포스팅 Retention Policy 제거 (모든 포스팅 보관)
...
- **18:40** 🗑️ [deprecate] posting-generator Worker 폐기 (make-page-subdomain에 통합됨)
...
- **10:15** ✨ [feature] Video 섹션 구현 (YouTube/Drive/TikTok/Instagram 지원, 모바일 최적화 16:9)
- **09:59** 📝 [docs] description 컬럼 사용 방침 명시 (AI 반영 필수)
...
- **02:50** 🐛 [bugfix] 코드 품질 체크 오탐 수정 + 백업 파일 삭제
- **02:48** 🐛 [bugfix] GitHub Actions YAML 구문 오류 수정
- **02:46** ✨ [feature] Google Drive → Sheets 자동화 Worker 추가
- **02:27** ⚡ [improvement] 코드 품질 규칙 변경 - 500줄 제한 제거, 단일 책임 원칙 적용
- **02:00** ⚡ [improvement] GitHub Actions 코드 품질 자동 검증 추가
- **01:53** ⚡ [improvement] Worker 코드 대폭 간소화 (5540줄 → 526줄)
- **01:53** ⚡ [improvement] Worker 코드 대폭 간소화 (5540줄 → 526줄)
- **01:19** 🐛 [bugfix] 템플릿 리터럴 중첩 구문 에러 수정
- **01:19** 🐛 [bugfix] 템플릿 리터럴 중첩 구문 에러 수정
- **01:05** ✨ [feature] Google Sheets 동적 연동 구현
- **00:36** ✨ [feature] Google Sheets 컬럼명 한글화
- **00:36** ✨ [feature] Google Sheets 컬럼명 한글화
- **00:23** 📝 [docs] Google Sheets 연동 반영, Supabase 폐기 문서화
...
- **22:44** ⚡ [improvement] 피아노 이모지 삭제 및 상상피아노 텍스트 수직 중앙 정렬
- **22:39** 🗑️ [deprecate] 사용하지 않는 Deploy Preview 워크플로우 삭제
- **22:38** ✨ [feat] 갤러리 섹션 Info로 변경 및 부제목 삭제
- **22:38** ✨ [feat] 갤러리 섹션 Info로 변경 및 부제목 삭제
- **22:38** ✨ [feat] 갤러리 섹션 Info로 변경 및 부제목 삭제
- **22:35** 📝 [docs] PROJECT.md 업데이트 및 자동화 워크플로우 추가
- **22:33** ⚡ [improvement] 모바일/PC 동일한 레이아웃으로 통일
- **22:33** ⚡ [improvement] 모바일/PC 동일한 레이아웃으로 통일
- **22:30** ✨ [feat] 00001 서브도메인 UI 업데이트
- **22:30** ✨ [feat] 00001 서브도메인 UI 업데이트
- **22:21** 🔹 [other] 클라우드플레어 워커 자동 배포
- **22:21** 🐛 [fix] GitHub Actions CLOUDFLARE_API_TOKEN 환경변수 추가
- **22:19** ✨ [feature] Cloudflare Workers 추가 (00001 서브도메인)
- **19:51** ✨ [feat] GitHub Actions 워크플로우 추가
