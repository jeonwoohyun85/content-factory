...
async function generateClientPage(client, debugInfo, env) {
  ...
  links.push({ icon: '📊', text: texts.stats || '통계', url: '/stats' });
...
export default {
  async scheduled(event, env, ctx) {
    ...
    try {
      ...
      // 90일 이상 오래된 통계 데이터 삭제
      try {
        await env.ANALYTICS_DB.prepare(
          'DELETE FROM analytics_visits WHERE visited_at < ?'
        ).bind(Math.floor(Date.now() / 1000) - 7776000).run();
  ...
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      */
      ...
      // 통계 SPA 페이지
      if (pathname === '/stats') {
        return new Response(generateAnalyticsSPA(subdomain, client.site_title || client.site_name || '통계'), {
          headers: {
